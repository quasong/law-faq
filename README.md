...
Web UI 會逐字呈現本地模型輸出、顯示可展開的法規來源，且每次新問題會取代上一題，不保存對話歷史。使用者可以從「回答模型」欄位選擇已安裝模型，或輸入其他 Ollama 模型名稱；若模型尚未安裝，介面會先請求確認，再串流顯示下載進度，完成後自動開始回答。若 Ollama 尚未連線，介面會顯示「啟動 Ollama」按鈕，點擊後執行本機 `ollama serve` 並自動等待連線恢復。
