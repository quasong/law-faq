...
Web UI 會逐字呈現本地模型輸出、顯示可展開的法規來源，且每次新問題會取代上一題，不保存對話歷史。「回答模型」下拉選單會把本機已安裝模型排在前面並標示實際大小，再從 Ollama 官方目錄載入可下載的文字生成模型；若模型尚未安裝，介面會先請求確認，再串流顯示下載進度，完成後自動開始回答。若官方目錄暫時無法連線，已安裝模型與內建推薦模型仍可使用。若 Ollama 尚未連線，介面會顯示「啟動 Ollama」按鈕，點擊後執行本機 `ollama serve` 並自動等待連線恢復。

基於安全考量，啟動 Ollama 與模型部署端點只接受從 `localhost` 發出的請求，而且自動啟動僅適用於本機 `OLLAMA_BASE_URL`。若把問答網站公開到網路，訪客不能透過這些端點啟動程序、任意下載模型或消耗主機磁碟空間。
...
- `GET /models/catalog`：讀取並快取 Ollama 官方可下載模型目錄
- `POST /ollama/start`：啟動本機 Ollama 服務，僅限本機呼叫
